...
public class ToDownJob {
		...
		for (int m = 0; m < allPage; m++) {
			...
			if (rs != null && row > 0) {
				...
				while (rs.next()) {
					...
					String comName=nibo.getComname();
					try {
						System.out.println("无注册公司基本信息 第" + i + "条");

						//查询该企业是否已经存入
						int listCBI = getcbi.getCBI(comName, session29);
						if(listCBI == 0){
						...
						//判断该公司职位是否已经导入
						int listcp = getcp.getCP(comName, nibo.getPosname(), session29);
						if(listcp == 0){
	 ...
	 *             异常说明
	 */
	public static void checkPosition(int comId, String comName, Session session, NingboEntity nibo, GetComPosition getcp) {
		int listCP = getcp.getCP(comName, nibo.getPosname(), session);
		if (listCP == 0) {
			// 发布职位
